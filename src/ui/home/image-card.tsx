        ...
        <div className="grid gap-4 m-6">
            <div>
                <div className="relative">
                    <Image src={main_image.url} width={300} height={200} className="h-auto max-w-full rounded-lg w-[100%] h-auto select-none " alt="" />
                    <div className="absolute m-2 bottom-0 p-2 text-sm text-cyan-800 rounded-lg bg-cyan-50 dark:bg-cyan-800 dark:text-cyan-300 opacity-65 hover:opacity-100 cursor select-none" role="alert">
                        <a href={"/" + main_image.url} className="font-medium">See on Map</a> {main_image.description}
                    </div>
                </div>

                <div className="grid grid-cols-5 gap-4 mt-3">
                    {image_set.map(({ url, description }) => (
                        <div key={url} onClick={() => switch_img({ url, description })} className={`${
                            main_image.url === url ? 'ring ring-cyan-500 ring-offset-2 rounded-lg' : ''
                        }`}>
                            <Image
                                src={url}
                                alt={description}
